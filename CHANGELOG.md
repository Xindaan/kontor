...
- **`real_prints` now verifies the quotation currency instead of declaring it.**
  The old `currency=` argument was passed to the data loader, where it only
  selects which FX series get attached — the price frame stays native. For
  tickers that happen to quote in the requested currency the result was correct,
  so the gap went unnoticed; for a pence-quoted line the raw value came back
  looking like euros, off by roughly a factor of 80. The argument is now called
  `source_currency`, defaults to `None` (no expectation, unchanged behaviour),
  and raises when it disagrees with the currency the loader determined.
  `currency=` remains as a deprecated alias so no existing caller silently gets
  something different than before.

### Removed

- **The optional `sentiment-finbert` dependency group.** It pulled the entire
  CUDA/torch tree into the lock file (37 packages) along with a standing set of
  `transformers` advisories, while never being installed. A major version bump
  would not have been covered by the test suite — nothing exercises an
  uninstalled optional group — so it would have traded a visible advisory for an
  invisible break. `FinBERTSentimentEngine` and the `--news-engine finbert`
  choice are unchanged; the lazy import now points at
  `pip install transformers torch`, which also means the version gets picked
  when the engine is actually needed rather than frozen months in advance.
...
- **Dependency updates closing 48 of 51 known advisories**, in two passes on the
  same day. All of them were transitive — none was a package this project asks
  for directly.
  - *Straightforward bumps:* `pillow` 12.1.0 → 12.3.0 (the largest single block),
    `protobuf` 6.33.2 → 7.35.1, `idna` 3.11 → 3.18, `soupsieve` 2.8.1 → 2.9.2.
    No constraint changed; only the resolved lock moved.
  - *Major-version work:* `cryptography` 46.0.3 → 50.0.0 (via `pdfminer-six`),
    `starlette` 0.36.3 → 1.6.0 together with `fastapi` 0.109 → 0.141 (they move as
    a pair), `pdfplumber` 0.10.4 → 0.11.10, `uvicorn` 0.27 → 0.52, and the test
    tooling (`pytest` 7.4 → 9.1, `pytest-cov` 4.1 → 7.1). Verified by running the
    suite against these versions rather than against whatever happened to be
    installed: **1175 passed**.
  - `python-multipart` 0.0.6 → 0.0.32 also **drops its caret constraint**. Under
    Poetry, `^0.0.6` permits nothing above `0.0.7`, which is why this single
    package accumulated 9 advisories — every fix needed a manual edit rather than
    an update. It is now `>=0.0.32,<0.1.0`.
- **Three advisories remain**, all in `transformers`, which reaches you only
  through the optional `sentiment-finbert` group — you do not have it unless you
  asked for it. Beyond that, the practical exposure of this project was always
  modest: it is a locally-run analysis tool, the web UI binds to localhost, and
  the image library is never imported by this codebase — it arrives only as a
