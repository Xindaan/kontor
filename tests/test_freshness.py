...
def _fake_yahoo(monkeypatch, frame, volumes=None, currency=None):
    def fake(**kwargs):
        cols = [t for t in kwargs['tickers'] if t in frame.columns]
        # `currency` is what the LOADER determined (provider metadata plus the
        # override table), not what the caller asked for -- real_prints checks
        # against this.
        resolved = dict(currency or {})
        return PriceData(prices=frame[cols].copy(),
                         volumes=None if volumes is None else volumes[cols].copy(),
                         currency={c: resolved.get(c, 'EUR') for c in cols})
...
class TestSourceCurrencyIsVerified:
    """The parameter used to declare a currency and convert nothing.

    ``currency=`` was handed to ``DataLoader.yahoo``, where it only selects the
    FX series to attach -- ``.prices`` stays native. For tickers that happen to
    quote in the requested currency the result was right, so it went unnoticed;
    for a pence-quoted line the raw value came back looking like euros, off by
    a factor of ~80.
    """

    IDX = pd.to_datetime(['2026-07-15', '2026-07-16'])

    def test_pence_line_requested_as_eur_raises(self, monkeypatch):
        """The real case. Previously: the raw pence value, labelled as euros."""
        _fake_yahoo(monkeypatch,
                    pd.DataFrame({'LEV3.L': [13400.0, 13519.5]}, index=self.IDX),
                    currency={'LEV3.L': 'GBp'})
        with pytest.raises(FreshnessError, match='does NOT convert'):
            real_prints('LEV3.L', '2026-06-01', '2026-07-18',
                        source_currency='EUR', today=FR)

    def test_matching_currency_passes(self, monkeypatch):
        _fake_yahoo(monkeypatch,
                    pd.DataFrame({'CORE.DE': [720.0, 724.0]}, index=self.IDX),
                    currency={'CORE.DE': 'EUR'})
        s = real_prints('CORE.DE', '2026-06-01', '2026-07-18',
                        source_currency='EUR', today=FR)
        assert float(s.iloc[-1]) == 724.0

    def test_without_expectation_behaviour_is_unchanged(self, monkeypatch):
        """Default None. Otherwise every existing caller wanting the native
        series would break -- and that is nearly all of them."""
        _fake_yahoo(monkeypatch,
                    pd.DataFrame({'LEV3.L': [13400.0, 13519.5]}, index=self.IDX),
                    currency={'LEV3.L': 'GBp'})
        s = real_prints('LEV3.L', '2026-06-01', '2026-07-18', today=FR)
        assert float(s.iloc[-1]) == 13519.5, 'native, unconverted -- as before'

    def test_deprecated_alias_still_works(self, monkeypatch):
        """`currency=` stays an alias, so no existing caller silently gets
        something different than before -- which is what the rename prevents."""
        _fake_yahoo(monkeypatch,
                    pd.DataFrame({'LEV3.L': [13400.0, 13519.5]}, index=self.IDX),
                    currency={'LEV3.L': 'GBp'})
        with pytest.raises(FreshnessError, match='does NOT convert'):
            real_prints('LEV3.L', '2026-06-01', '2026-07-18', currency='EUR', today=FR)
