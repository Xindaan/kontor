...
> What the framework does **not** do is listed under [Limitations](#limitations) —
...
**Optional extras.** The core install is deliberately lean; anything with a heavy
or rarely-needed dependency lives in an optional group. Install what you need:

| Group | Command | Needed for |
|---|---|---|
| `web` | `poetry install --with web` | the web UI (`backtest web`) — **required for Option A below** |
| `ml` | `poetry install --with ml` | ML forecast tilt, regime classifier (lightgbm, xgboost, scikit-learn) |
| `sentiment` | `poetry install --with sentiment` | `--news-engine vader` (nltk; also run `python -m nltk.downloader vader_lexicon` once) |
| `sentiment-finbert` | `poetry install --with sentiment-finbert` | `--news-engine finbert` (pulls torch + transformers, a large download) |

Without its group, a command exits with an explicit install hint rather than a
stack trace — an `Error: ... dependencies not installed` message means you are
missing an extra, not that the install is broken.
...
```bash
poetry install --with web   # once — the web deps are an optional group
...
> **You have to create this file yourself.** It describes *your* holdings, so no
> portfolio JSON ships with the repository — `data/manual/` contains only
> `provenance.json`. Wherever the examples below say `--portfolio
> data/manual/portfolio.json`, substitute the path to the file you wrote. The
> same goes for the other example inputs (`meta_candidates.json`, the
> SeekingAlpha CSV, `results/signals_report.json`): they are placeholders for
> your own data or the output of an earlier step, not files you will find after
> cloning.
...
backtest meta-promotion [STRATEGIES...] [OPTIONS]
  ...
  --soxl-proxy                  Long-history research mode: use SOXL as proxy
...
# API keys: the vendor datasets read real environment variables. There is no
# .env loading (no python-dotenv), so export them in your shell — a .env file
# would be ignored:
#   export FINNHUB_API_KEY=...     # finnhub_analyst_*, finnhub_news
#   export NEWSAPI_API_KEY=...     # newsapi_news
# Everything marked mock_/synthetic_ below needs no key at all and is the way to
# try the pipeline without signing up anywhere.
...
│   ├── --baseline PATH                  Incumbent baseline (default: Sticky/Levered Live)
│   ├── --soxl-proxy                     Long-history research mode (SOXL as proxy)
│   ├── --start DATE                     Backtest start (default: 2016-01-01)
│   ├── --end DATE                       Backtest end (default: today)
│   ├── --capital FLOAT                  Initial capital EUR (default: 10000)
│   ├── --costs FLOAT                    Transaction costs (default: 0.001)
...
| **Cost Basis** | FIFO (First In, First Out) — the only method implemented; `cost_basis_method` other than `"FIFO"` is **rejected at construction** rather than silently run as FIFO |
...
The default backtest is deliberately frictionless-ish (a flat percentage cost, a
snap-to-target rebalance). That is fine for ranking strategies, but it overstates
what reaches a real German depot. These `BacktestConfig` options close the gap
between the simulation and how the book is actually traded and billed. They are
Python-level knobs (constructor parameters), and **the three below are off by
default**, so switching to this file's defaults leaves existing runs
bit-identical.

> Note that "off by default" applies to these three only — it is not a claim that
> the backtest is frictionless out of the box. `slippage_pct` defaults to `0.0005`
> and `slippage_bps` to `5.0`, so every run already carries some friction. The
> full field list is in [Configuration Reference](#configuration-reference).

### Broker commission model

The transaction-cost model brackets the percentage tier with both a floor and a
**ceiling** (`commission_max`), which is how real retail schedules work. Two
German broker profiles are provided from the brokers' own published fee tables:

```python
from backtest.costs import TransactionCostModel

db = TransactionCostModel.deutsche_bank_maxblue()  # 0.25%, min €8.90, max €58.90
tr = TransactionCostModel.trade_republic()         # flat €1.00 per order

db.calculate(1_000).commission     # 8.90  (floored)
db.calculate(10_000).commission    # 25.00 (in the tier)
db.calculate(100_000).commission   # 58.90 (capped)
```

Without the cap, a €100k order priced at 0.25% would charge €250 — over four
times what the depot is actually billed. `commission_max=0` (the default) means
no cap.

### No-trade band

`rebalance_min_deviation` makes the backtester trade only when the largest gap
between an **actual** and a **target** weight breaches the band — then it
rebalances fully. Without it, the engine snaps to target at *every* rebalance
date (down to a one-cent difference), simulating a weekly churn that a live rule
("only trade when drift breaches the gate") never produces.

```python
BacktestConfig(rebalance_frequency="weekly", rebalance_min_deviation=0.20)
```

On one levered sleeve, roughly **78% of trades and 23% of the tax paid** at a
0.20 band came from that phantom snap-to-target — trades that never happen live.
`0.0` (the default) keeps the old snap-to-target behaviour.

### Rebalance weekday anchor

For weekly rebalancing, `rebalance_weekday` pins execution to a fixed weekday
(`0`=Mon … `4`=Fri) instead of the ISO-week's last trading day (≈ Friday). If the
anchor day is a market holiday, the next open day in the same week is used.

```python
BacktestConfig(rebalance_frequency="weekly", rebalance_weekday=0)  # Monday
```

This is **executional, not an edge**: across one study the weekday moved CAGR by
~0.6 points but underwater duration by over 3 years, so pinning it to the day you
actually trade matters for tail-risk figures even when it barely moves the return.
`None` (the default) keeps the legacy last-trading-day behaviour.

---

## Configuration Reference

Every field of `BacktestConfig` (`src/backtest/backtester.py`), with its default.
Most have a CLI equivalent; the ones that do not are set from Python. Anything not
listed here does not exist — if a knob you expect is missing, that is the answer.
...
| `currency` | `"EUR"` | Portfolio currency; prices are converted into it |
| `benchmark` | `"S&P 500"` | Comparison series; `None` disables it |
| `risk_free_rate` | `0.02` | Annual rate used for **Sharpe/Sortino**. Change it and every risk-adjusted metric moves |
| `cash_rate` | `0.0` | Interest earned on uninvested cash |

**Costs and execution** — see [Execution & Cost Realism](#execution--cost-realism)

| Field | Default | Meaning |
|---|---|---|
| `costs_pct` | `0.001` | Flat commission as a fraction of trade value (CLI: `--costs`) |
| `slippage_pct` | `0.0005` | Execution slippage. **Not zero by default** — every run carries it |
| `spread_pct` | `0.0` | Half bid-ask spread charged per trade |
| `cost_profile` | `None` | Per-ticker cost overrides; falls back to the flat values above |
| `execution_lag_days` | `0` | Trading days between signal and execution (`1` avoids same-close lookahead) |

**Liquidity limits**

| Field | Default | Meaning |
|---|---|---|
| `max_volume_participation` | `None` | Cap an order at this fraction of the day's volume |
| `min_daily_dollar_volume` | `0.0` | Skip tickers whose daily turnover is below this |
| `liquidity_on_missing_volume` | `"allow"` | What to do when volume data is absent: `allow` or `skip` |

**Rebalancing**

| Field | Default | Meaning |
|---|---|---|
| `rebalance_frequency` | `None` | `daily` / `weekly` / `monthly` / `quarterly` / `yearly`; `None` uses the strategy's own |
| `rebalance_min_deviation` | `0.0` | No-trade band on \|actual − target\|; `0.0` snaps to target every date |
| `rebalance_weekday` | `None` | Weekly anchor day, `0`=Mon … `4`=Fri |

**German tax model** — see [German Tax Model](#german-tax-model)

| Field | Default | Meaning |
|---|---|---|
| `tax_enabled` | `True` | Master switch (CLI: `--no-tax`) |
| `tax_rate` | `0.26375` | Abgeltungssteuer incl. Solidaritätszuschlag |
| `tax_partial_exemption` | `0.30` | Teilfreistellung for equity funds |
| `tax_exemption_amount` | `1000.0` | Freistellungsauftrag (CLI: `--tax-exemption`) |
| `cost_basis_method` | `"FIFO"` | Only `"FIFO"` is implemented; anything else raises |
| `daily_liquidation_tax` | `True` | Value the daily curve net of the latent liquidation tax, not just realized tax |
| `equity_fund_map` | `None` | Ticker → is-equity-fund; controls who gets the Teilfreistellung |
| `tax_treatment_map` | `None` | Ticker → (instrument class, is-equity-fund) for finer control |
| `actual_liquidation_at_end` | `False` | Really sell everything on the last day (CLI: `--liquidate-at-end`) |

**Metrics, validation, plumbing**

| Field | Default | Meaning |
|---|---|---|
| `metric_basis` | `"net_liquidation"` | `gross`, `net_realized` or `net_liquidation` |
| `allow_universe_lookahead` | `False` | Permit non-point-in-time universes (survivorship bias) |
| `validate` | `True` | Run pre/post validation checks |
| `strict_validation` | `False` | Turn validation warnings into errors |
| `drip_enabled` | `False` | Reinvest dividends (CLI: `--drip`) |
| `exposure_policy` | `None` | Exposure-policy engine config |
| `risk_overlay` | `None` | Risk-overlay engine config |
| `external_features_loader` | `None` | Injects an external-features provider for the run |

### `CostConfig`

Used by `TransactionCostModel` and the `RunConfig` path
(`src/backtest/config/run_config.py`):

| Field | Default | Meaning |
|---|---|---|
| `commission_pct` | `0.001` | Commission as a fraction of trade value |
| `commission_min` | `0.0` | Floor per order |
| `commission_max` | `0.0` | Ceiling per order; `0.0` means no cap |
| `spread_bps` | `0.0` | Full bid-ask spread in bps (half is applied per trade) |
| `slippage_bps` | `5.0` | Slippage in bps. **Not zero by default** |
...
The individual points are stated where they belong throughout this file; they are
collected here so you do not have to find them one at a time.

**The tax model is a good model, not your tax return.** It covers
Abgeltungssteuer, Solidaritätszuschlag, Teilfreistellung, Freistellungsauftrag and
the two loss pots on FIFO lots. It excludes **Vorabpauschale**,
**Kirchensteuer** and the **derivatives loss pot** (§20 Abs. 6 Satz 5 EStG), and
only FIFO cost basis is implemented. It is not tax advice and it will not match a
Steuerbescheid to the cent.

**A backtest is not a forecast.** Every figure quoted in this file comes from one
study over one window with one parameter set. Change the period, the weekday, the
starting year, and the numbers move — sometimes by more than the effect being
measured (that is the point of the [rebalance weekday](#rebalance-weekday-anchor)
example). Treat quoted improvements as "this is what the mechanism did there",
never as an expected return. The [Best Practices](#best-practices) section exists
because the default failure mode of a backtesting framework is a convincing
overfit.

**Costs and execution are modelled, not observed.** Commission schedules, spread
and slippage are parameters you supply. Real fills differ; illiquid instruments
differ a lot. The liquidity caps help, they do not make the result real.

**Prices come from free public sources.** Yahoo Finance data carries survivorship
and revision quirks; the [Data Integrity](#data-integrity) section describes the
guards, which detect problems rather than eliminate them. Fundamental data is
sparse and, in this repository, largely synthetic sample data.

**Nothing here places an order.** The live path derives *plan-only* order sheets;
no broker integration submits anything, by design.

**No performance claim is being made.** This repository ships a framework and a
strategy library, not a track record. Strategies included as examples — including
the levered-momentum ones — can and do have deep drawdowns; run them over
2000-2003 and 2007-2009 before drawing conclusions.
