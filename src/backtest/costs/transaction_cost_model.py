...
class TransactionCostModel:
    ...
    Attributes:
        ...
        commission_max: Maximum commission per trade (0 = no cap)
        spread_bps: Bid-ask spread in basis points
        slippage_bps: Slippage in basis points
    """
    commission_pct: float = 0.001  # 0.1%
    commission_min: float = 0.0
    commission_max: float = 0.0  # 0 = no cap
    ...
    def calculate(
        ...
        """
        # Commission: max of (percentage * value, minimum), then optional cap.
        # Real retail schedules bracket the percentage tier with both a floor and
        # a ceiling (e.g. Deutsche Bank / Maxblue: 0.25%, min 8.90, max 58.90).
        commission = max(
    ...
    def from_config(cls, config) -> "TransactionCostModel":
        ...
        return cls(
            ...
            commission_max=getattr(cost_config, 'commission_max', cost_config.get('commission_max', 0.0)),
    ...
    # German retail broker profiles. The commission figures below are the
    # brokers' own published fee schedules (public information), reproduced here
    # so a EUR backtest can price trades the way the depot actually gets billed.
    @classmethod
    def deutsche_bank_maxblue(cls) -> "TransactionCostModel":
        """Deutsche Bank / Maxblue online schedule: 0.25% of trade value,
        floored at EUR 8.90 and capped at EUR 58.90 per order."""
        return cls(
    ...
    @classmethod
    def trade_republic(cls) -> "TransactionCostModel":
        """Trade Republic: a flat EUR 1.00 external settlement fee per order,
        modelled as a fixed commission (min == max)."""
        return cls(
            commission_pct=0.0,
            commission_min=1.0,
            commission_max=1.0,
