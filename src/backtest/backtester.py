...
class BacktestConfig:
    ...
    # NO-TRADE BAND: only rebalance when the largest gap between an actual and a
    # target weight breaches this threshold -- then rebalance fully. Until this
    # existed the backtester snapped to target at EVERY rebalance date (down to a
    # one-cent difference), which simulates a weekly churn that a real live rule
    # ("only trade when actual-vs-target drift breaches the gate") never produces.
    # 0.0 = old behaviour (default, does not change existing runs).
    rebalance_min_deviation: float = 0.0
    # REBALANCE WEEKDAY ANCHOR (weekly frequency only): pin weekly rebalances to a
    # fixed weekday (0=Mon .. 4=Fri). On a market holiday the next open day in the
    # same week is used, else the last open day before it. None = legacy behaviour
    # (the resampled period end). The choice is executional, not an edge -- the
    # anchor day materially shifts underwater duration, so it must be explicit.
...
class Backtester:
    ...
    def _run_inner(self) -> BacktestResult:
        ...
        rebalance_dates = generate_rebalance_dates(
            daily_prices.index, rebalance_frequency,
            weekday=getattr(self.config, 'rebalance_weekday', None))
        ...
        for idx, date in enumerate(rebalance_dates):
            ...
            # NO-TRADE BAND (rebalance_min_deviation): only trade when the largest
            # gap between an actual and a target weight breaches the gate -- then
            # rebalance fully. This mirrors the live rule; without it the run
            # simulates a weekly snap-to-target that never happens in reality.
            if self.config.rebalance_min_deviation > 0.0:
                band_total = portfolio.total_value(execution_prices)
                if band_total > 0:
                    actual_w = {t: v / band_total
                                for t, v in portfolio.position_values(execution_prices).items()}
                    target_w = dict(target_allocation.weights)
                    max_dev = max(
                        (abs(target_w.get(t, 0.0) - actual_w.get(t, 0.0))
                         for t in set(target_w) | set(actual_w)),
                        default=0.0,
                    )
                    if max_dev <= self.config.rebalance_min_deviation:
                        # No trade: extend the daily curve to the next rebalance, skip.
                        skip_next = (rebalance_ilocs[idx + 1] if idx + 1 < len(rebalance_ilocs)
                                     else len(daily_prices) - 1)
                        _record_daily_segment(execution_iloc + 1, skip_next + 1, cumulative_tax_paid)
                        continue
