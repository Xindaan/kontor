...
class Portfolio:
    ...
    def has_holdings(self) -> bool:
        """Does this portfolio state actually carry any position?

        A portfolio with NO positions is not a portfolio at weight 0 -- it is an
        UNKNOWN holding. The distinction matters: get_weights() returns 0 in both
        cases, and a drift computed against that reads as a full-size target, i.e.
        an urgent BUY for the whole book. That is a fail-open trap in the same
        class as a stale price: it produces a confident wrong signal rather than
        an error. Callers use this to fail loud instead.
        """
        return any(shares > 0 for shares in self.positions.values())
...
class SignalGenerator:
    ...
    def generate(
        ...
        # An empty holding is NOT a holding of 0 -- otherwise every target reads as
        # a full-size drift/BUY. Fail loud instead of fail open (same class as a
        # stale price producing a confident wrong answer).
        if not self.portfolio.has_holdings():
            price_warnings = list(price_warnings) + [
                "NO HOLDINGS LOADED -- the drift / BUY columns are MEANINGLESS. "
                "They show the target weights against an empty portfolio, not against your book."
            ]
